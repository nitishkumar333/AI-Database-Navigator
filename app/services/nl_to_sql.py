...
The previous query failed with error: {str(e)}
Previous query was: {result['sql_query']}

Generate a corrected SQL query:"""

            try:
                response = llm.invoke(retry_prompt)
                retry_sql = response.content.strip()
                retry_sql = refine_sql_from_markdown(retry_sql)

                retry_result = validation.validate_sql_query(retry_sql)
                
                if retry_result['validation_result'].get('is_safe') and retry_result['validation_result'].get('schema_validated'):
                    with engine.connect() as connection:
                        query_result = connection.execute(text(retry_result['sql_query']))
                        columns = list(query_result.keys())
                        rows = [dict(zip(columns, row)) for row in query_result.fetchall()]
                    ...
                    return {
                        "success": True,
                        "question": question,
                        "generated_sql": retry_result['sql_query'],
                        "columns": columns,
                        "rows": rows,
                        "row_count": len(rows),
                        "latency_ms": round(latency_ms, 2),
                    ...
                    return {
                        ...
                        "error": retry_result['validation_result'].get('explanation', 'Validation Failed'),
                        "latency_ms": round(latency_ms, 2),
                        "relevant_tables": all_tables,
                    }
